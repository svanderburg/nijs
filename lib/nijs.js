...
var NixFunction = require('./NixFunction.js').NixFunction;
        ...
        if(arg === null) {
            ...
            switch(type) {
                ...
                case "object":
                    if(Array.isArray(arg)) { /* Arrays are also objects, but this will tell us the difference */
                    ...
                    } else if(arg instanceof NixFunction) {
                        expr += "(";
                        
                        /* Generate the function header */
                        if(typeof arg.argSpec == "string")
                            expr += arg.argSpec + ": ";
                        else {
                            expr += "{";
                            
                            if(Array.isArray(arg.argSpec)) {
                                
                                for(var j = 0; j < arg.argSpec.length; j++) {
                                    expr += arg.argSpec[j];
                                    
                                    if(j < arg.argSpec.length - 1)
                                        expr += ", ";
                                }
                            } else {
                                ...
                                for(var varName in arg.argSpec) {
                                    var value = arg.argSpec[varName];
                        ...
                        expr += jsToNix(arg.body);
                        expr += ")";
                        
                        break;
                    
                    } else if(arg instanceof NixFunInvocation) {
                        expr += "(" + jsToNix(arg.funExpr) + " " + jsToNix(arg.paramExpr) + ")";
                        break;
