...
As can be observed, a function proxy has a very simple structure. It returns an
object that can be converted into a semantically equivalent (or similar) Nix
language construct through a function called `jsToNix()`.
...
JavaScript objects are translated into Nix expression language objects as
follows:

* `null` references are translated into `null` values
* Objects of type `boolean` and `number` are translated verbatim
* Objects of type `string` and `xml` are translated verbatim and automatically escaped
...
JavaScript. Nonetheless, they can still be generated by composing an abstract
syntax tree of objects that are instances of prototypes that inherit from
`NixObject`:
...
* Referring to existing Nix store paths can be done by creating an object instanceof `NixStorePath`.
...
* Some attributes can be imported into the lexical scope of a function by creating a `NixWith` object.
* To define build instructions in JavaScript (as opposed to embedded bash code in strings) can be done by creating a `NixInlineJS` object (see section: Writing inline JavaScript code in a NiJS package specification) for more information.
