...
* Object members whose values are `undefined` are not included in a generated attribute set. In all other cases `undefined` translates to `null`.
...
* Referring to existing Nix store paths can be done by creating an object that is an instance of `NixStorePath`.
...
* Defining build instructions in JavaScript (as opposed to bash code embedded in strings) can be done by creating a `NixInlineJS` object (see section: Writing inline JavaScript code in a NiJS package specification for more information).
